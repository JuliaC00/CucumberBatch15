...
Background:
  ...
  Scenario: create an employee using API call
    ...
    Then the status code for featuring an employee is 201
    Then employee contains key "Message" and  value "Employee Created"
  ...
  @dynamic
  Scenario: create an employee using API call
      Given request is prepared to create an employee with dynamic data "Julia" , "Cherviakova" , "A" , "F" , "2000-06-07" , "Confirmed" , "Mechanical Engineer"
    When a POST call is made to create an employee
    Then the status code for creating an employee is 201
    Then the employee contains key "Message" and  value "Employee Created"
    Then the employee id "Employee.employee_id" is stored as a global variable to be used for other calls
